# EvDNN-CP with rolling window  # noqa: INP001

"""Conformal prediction post-processing for evidential DNN forecasts."""
...
model_name = "EvDNN"
with open(
    f"notebooks/evaluate_models/results/metric_evaluation/{model_name}.pkl", "rb"
) as f:
...
for pred_val, pred_test in zip(pred_val_list, pred_test_list, strict=True):
    ...
    nonconformity_scores = np.concatenate(
        [np.abs(residuals_val), np.abs(residuals_test)]
    )
...
EvDNN_CP = {}
EvDNN_CP["model_name"] = "EvDNN-CP"
EvDNN_CP["prediction"] = pred_test
EvDNN_CP["quantile"] = percentiles_test

model_dict = []
model_dict.append(EvDNN_CP)
