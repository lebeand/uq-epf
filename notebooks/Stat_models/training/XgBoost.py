...
"""XGBoost training and rolling-window evaluation over multiple horizons."""
...
# 1) XGBoost for different horizons
...
for i in range(num_val):
    ...
    reg_xgb_1461.fit(X=x_train_val[unten:oben, :], y=y_train_val[unten:oben, :])
    pred_val_1461[i, :] = reg_xgb_1461.predict(X=x_train_val[[oben + 1], :])
print("1461 window: ", np.mean(np.abs((pred_val_1461 - y_val) * scale)))

pred_val_1092 = np.zeros((num_val, 24))
for i in range(num_val):
    reg_xgb_1092 = XGBRegressor(
        n_estimators=n_estimators,
        learning_rate=learning_rate,
        max_depth=max_depth,
        min_child_weight=min_child_weight,
    ...
    oben = num_train_val - num_val + i - 1
    reg_xgb_1092.fit(X=x_train_val[unten:oben, :], y=y_train_val[unten:oben, :])
    pred_val_1092[i, :] = reg_xgb_1092.predict(X=x_train_val[[oben + 1], :])
print("1092 window: ", np.mean(np.abs((pred_val_1092 - y_val) * scale)))


pred_val_84 = np.zeros((num_val, 24))
for i in range(num_val):
    reg_xgb_84 = XGBRegressor(
        n_estimators=n_estimators,
        learning_rate=learning_rate,
        max_depth=max_depth,
        min_child_weight=min_child_weight,
    ...
    oben = num_train_val - num_val + i - 1
    reg_xgb_84.fit(X=x_train_val[unten:oben, :], y=y_train_val[unten:oben, :])
    pred_val_84[i, :] = reg_xgb_84.predict(X=x_train_val[[oben + 1], :])
print("84 window: ", np.mean(np.abs((pred_val_84 - y_val) * scale)))

pred_val_56 = np.zeros((num_val, 24))
for i in range(num_val):
    reg_xgb_56 = XGBRegressor(
        n_estimators=n_estimators,
        learning_rate=learning_rate,
        max_depth=max_depth,
        min_child_weight=min_child_weight,
    ...
    oben = num_train_val - num_val + i - 1
    reg_xgb_56.fit(X=x_train_val[unten:oben, :], y=y_train_val[unten:oben, :])
    pred_val_56[i, :] = reg_xgb_56.predict(X=x_train_val[[oben + 1], :])
...
for i in range(num_test):
    ...
    pred_test_1461[i, :] = reg_xgb_1461.predict(X=x_train_val_test[[oben + 1], :])
...
for i in range(num_test):
    ...
    pred_test_1092[i, :] = reg_xgb_1092.predict(X=x_train_val_test[[oben + 1], :])
...
for i in range(num_test):
    ...
    oben = num_total - num_test + i - 1
    reg_xgb_84.fit(X=x_train_val_test[unten:oben, :], y=y_train_val_test[unten:oben, :])
    pred_test_84[i, :] = reg_xgb_84.predict(X=x_train_val_test[[oben + 1], :])
...
for i in range(num_test):
    ...
    oben = num_total - num_test + i - 1
    reg_xgb_56.fit(X=x_train_val_test[unten:oben, :], y=y_train_val_test[unten:oben, :])
    pred_test_56[i, :] = reg_xgb_56.predict(X=x_train_val_test[[oben + 1], :])
...
reg_xgb_no_roll.fit(X=x_train, y=y_train)
pred_val_no_roll = reg_xgb_no_roll.predict(X=x_val)
pred_test_no_roll = reg_xgb_no_roll.predict(X=x_test)
print(
...
XGBoost_refit_4m = {}
XGBoost_refit_4m["model_name"] = "XGBoost"
XGBoost_refit_4m["prediction_val"] = pred_val_mean * scale + offset
XGBoost_refit_4m["prediction"] = (
    np.repeat(np.expand_dims(pred_test_mean, 0), 10, 0) * scale + offset
)
XGBoost_refit_4m["prediction_val_no_roll"] = pred_val_no_roll * scale + offset
XGBoost_refit_4m["prediction_no_roll"] = pred_test_no_roll * scale + offset
XGBoost_refit_4m["quantile"] = np.zeros((10, 99, num_test, 24))
XGBoost_refit_4m["prediction_val_1461"] = pred_val_1461 * scale + offset
XGBoost_refit_4m["prediction_val_1092"] = pred_val_1092 * scale + offset
XGBoost_refit_4m["prediction_val_84"] = pred_val_84 * scale + offset
XGBoost_refit_4m["prediction_val_56"] = pred_val_56 * scale + offset
XGBoost_refit_4m["prediction_1461"] = pred_test_1461 * scale + offset
XGBoost_refit_4m["prediction_1092"] = pred_test_1092 * scale + offset
XGBoost_refit_4m["prediction_84"] = pred_test_84 * scale + offset
XGBoost_refit_4m["prediction_56"] = pred_test_56 * scale + offset
XGBoost_refit_4m["offset"] = offset
XGBoost_refit_4m["scale"] = scale
XGBoost_refit_4m["y_val"] = y_val * scale + offset
XGBoost_refit_4m["y_test"] = y_test * scale + offset

model_dict = []
model_dict.append(XGBoost_refit_4m)

with open(  # noqa: PTH123
    f"notebooks/evaluate_models/results/archive/XGBoost_{n_estimators}est_lr{learning_rate}_md{max_depth}.pkl",
    "wb",
) as fp:
